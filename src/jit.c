...
static void peel_loop(struct JitState *state) {
	...
	for (unsigned i = 0; i < preamble_len; ++i) {
		if (i >= snap->ir_start) { // Copy-substitute the next snapshot
			...
			while (l < lend) *n++ = *l++;
			*s = (struct Snapshot)
				{ .ir_start = state->len, .offset = offset, .hotcount = 1,
...
static IrRef record_c_call(struct JitState *state, uintptr_t *bp, struct Instruction x) {
	...
	for (; intrinsic->name; ++intrinsic) {
		...
		a = SLOT(state, x.a + 2);
		if (intrinsic->arg1_ty != TY_ANY) guard_type(state, &a, intrinsic->arg1_ty);

		if (intrinsic->arg2_ty == (uint8_t) -1) goto out;
		b = SLOT(state, x.a + 2 + 1);
		if (intrinsic->arg2_ty != TY_ANY) guard_type(state, &b, intrinsic->arg2_ty);
...
bool jit_record(struct LispCtx *ctx, struct Instruction *pc, LispObject *bp) {
	...
	case TAIL_CALL: case TAIL_CALL_INTERPR:
		...
		if (/* Avoid reload? */ fn_value != *bp) *state->bp = state->bp[x.a];
