...
static bool is_mcode_ovf(struct RegAlloc *ctx) {
#ifndef NDEBUG
...
static void asm_loop(struct RegAlloc *ctx) {
	...
	FOR_ONES(reg, ctx->clobbers & ~(ctx->available | ctx->phi_regs))
		if (!is_mcode_ovf(ctx)) reload(ctx, ctx->reg_costs[reg]);
...
static void asm_proj(struct RegAlloc *ctx, Ref ref, size_t offset) {
	...
	asm_rr(&ctx->as, 1, IMM_GRP1_MR(XG_ADD), dst, REG_LISP_CTX);
	asm_rmrd(&ctx->as, 0, XI_MOVmr, dst, src, offset - 1);
}
		...
		if (ref < ctx.next_snapshot_beg) {
			...
			FOR_SNAPSHOT_ENTRIES(snapshot, state->stack_entries, e)
				if (IS_VAR(e->ref) && !IR_GET(state, e->ref).spill_slot
					&& !is_mcode_ovf(&ctx)) reg_use(&ctx, e->ref, -1);
		}

		union Node x = IR_GET(state, ref);
		if (is_mcode_ovf(&ctx)) goto err_realloc;
		switch (x.op) {
		case IR_SLOAD:
