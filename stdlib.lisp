  ...
  (let (listify
        ...
        f (fn (level x)
            (if (consp x)
              ...
              (list 'quote 'quote x))))
    (cdr (f 0 structure))))
(set unquote-splicing (defmacro (unquote _) (throw ()))) ; Invalid outside of quasiquote

;; set wrapper that helps with lambda indentation and will allow setting docstring.
(defmacro (def x . body)
  (cons 'set (if (consp x)
...
(def fib (let (f (fn (n) (if (< n 2) (if (< n 1) 0 1)
                             (+ (f (+ n -1)) (f (+ n -2))))))
           f))
;; (print (fib 25))
