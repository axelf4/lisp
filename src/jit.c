...
struct LispTrace {
	void (*f)(); ///< Machine code entry point.
	uint16_t len;
...
struct JitState *jit_new() { return calloc(1, sizeof(struct JitState)); }
...
/** Abort recording asynchronously, due to NYI (not yet implemented). */
[[gnu::cold]] static void nyi(struct JitState *state) { state->is_aborted = true; }
...
static IrRef emit_const(struct JitState *state, uint8_t ty, uintptr_t x) {
	...
	if (++state->num_consts >= MAX_CONSTS) { nyi(state); return 0; }
	IR_GET(state, i).v = x;
out: return ty << REF_TYPE_SHIFT | i;
}

static IrRef emit(struct JitState *state, union Node x) {
	if (state->end >= IR_BIAS + MAX_TRACE_LEN) { nyi(state); return 0; }
...
#define CONDFOLD(cond) do { if (!(cond)) nyi(state); return 0; } while (0)
...
static IrRef uref(struct JitState *state, uintptr_t *bp, uint8_t idx) {
	...
	struct Upvalue *upvalue = closure->upvalues[idx];
...
static void subst_snapshot(struct JitState *state, uintptr_t subst,
	...
	if (MAX_SNAPSHOT_ENTRIES - offset < lim) { nyi(state); return; }
	struct SnapshotEntry
...
static enum Register reload(struct RegAlloc *ctx, Ref ref) {
	...
	if (ref >= REF_FIRST) {
		if (!x->spill_slot && !(x->spill_slot = ++ctx->num_spill_slots))
			nyi(ctx->state); // Out of spill slots
		// Reload from stack
...
static void asm_type_guard(struct RegAlloc *ctx, Ref ref) {
	...
	asm_guard(ctx, CC_E ^ (ty == LISP_INTEGER));
	*--ctx->as.p = 0x1;
	asm_rr(&ctx->as, 0, XI_TESTmi8, 0, reg);
}

static void asm_store(struct RegAlloc *ctx, enum Register base, int32_t slot, uint8_t ty, Ref ref) {
	union Node x = IR_GET(ctx->state, ref);
	if (IS_VAR(ref)) {
		enum Register reg = reg_use(ctx, ref, ~(1 << base));
		asm_rmrd(&ctx->as, 1, XI_MOVmr, reg, base, slot * sizeof x.v);
	} else if (ty == LISP_INTEGER) {
		asm_write32(&ctx->as, x.v);
		asm_rmrd(&ctx->as, 0, XI_MOVmi, 0, base, slot * sizeof x.v);
	} else asm_mov_mi64(&ctx->as, base, slot * sizeof x.v, x.v);
}
...
static void asm_stack_restore(struct RegAlloc *ctx) {
	...
	FOR_SNAPSHOT_ENTRIES(snapshot, ctx->state->stack_entries, e)
		if (!is_asm_full(ctx)) asm_store(ctx, bp, e->slot, e->ty, e->ref);
}

static void asm_uload(struct RegAlloc *ctx, Ref ref, union Node x) {
	enum Register reg = reg_def(ctx, ref, -1), fn = reg_use(ctx, x.a, -1);
	asm_rmrd(&ctx->as, 1, XI_MOVrm, reg, reg, 0);
	asm_rmrd(&ctx->as, 1, XI_MOVrm, reg, reg, offsetof(struct Upvalue, location));
	asm_rmrd(&ctx->as, 1, XI_MOVrm, reg, fn, offsetof(struct Closure, upvalues[x.b]) - 1);
}
...
static void asm_loop(struct RegAlloc *ctx) {
	...
	FOR_ONES(reg, ctx->phi_regs) {
		...
		if (LIKELY(in->reg == reg) || is_asm_full(ctx)) continue;
	...
	FOR_ONES(reg, ctx->phi_regs) {
		Ref lref = ctx->phi_refs[reg];
		union Node *in = &IR_GET(ctx->state, lref);
		if (LIKELY(!in->spill_slot) || is_asm_full(ctx)) continue;
...
#define ARG_REGS (1 << rdi | 1 << rsi | 1 << rdx | 1 << rcx | 1 << r8 | 1 << r9)
	...
	for (unsigned i = x.b; i--;) {
		...
		union Node arg = IR_GET(ctx->state, ref + 1 + i);
		asm_store(ctx, args_reg, i, arg.ty, arg.a);
	}
	asm_rmrd(&ctx->as, 1, XI_LEA, args_reg, rsp, ctx->num_spill_slots * sizeof x.v);
	if (ckd_addassign(&ctx->num_spill_slots, x.b)) nyi(ctx->state);
...
static struct LispTrace *assemble_trace(struct JitState *state, enum TraceLink link_type) {
	...
	if (UNLIKELY(state->is_aborted)) return NULL;
...
do_retry:
	...
	*(ctx.as.p -= 1 + sizeof(int32_t)) = XI_JMP; // Loop backedge placeholder
	...
	for (Ref ref = state->end; ref-- > REF_FIRST;) {
		...
		union Node x = IR_GET(state, ref);
		if (is_nop(x)) continue; // Dead-code elimination (DCE)
		if (is_asm_full(&ctx)) goto err_realloc;
		if (x.ty & IR_GUARD) asm_type_guard(&ctx, ref);
		switch (x.op) {
		case IR_SLOAD:
		...
		case IR_GLOAD:
			...
			break;
		case IR_ULOAD: asm_uload(&ctx, ref, x); break;
		case IR_PLOAD:
		...
		case IR_CALL: asm_call(&ctx, ref, x); break;
	...
	FOR_ONES(reg, ctx.phi_regs) {
		...
		if (in.spill_slot && !(in.ty & IR_MARK)) nyi(state); // Spilled outside loop
	}
...
static void penalize(struct JitState *state) {
	...
	do if (slot->pc == state->start_pc) goto found; while (++slot < end);

	slot = &state->penalties[state->next_penalty_slot++ % LENGTH(state->penalties)];
	*slot = (struct Penalty) { .pc = state->start_pc, .value = TRACE_ATTEMPTS };
found:
	if (!--slot->value)
...
bool jit_record(struct LispCtx *ctx, struct Instruction *pc, LispObject *bp) {
	...
	case RET: do_ret:
		...
		if (!bp[-offset + 1]) { nyi(state); break; }
		struct Closure *closure = UNTAG_OBJ(bp[-offset]);
		emit_const(state, LISP_BYTECODE, TAG_OBJ(prototype_chunk(closure->prototype)));
		...
		for (IrRef ref = state->chain[IR_RET]; ref; ref = IR_GET(state, ref).prev)
			if (!state->parent && IR_GET(state, ref).a == pc_ref) nyi(state);
		break;
	case FHDR:
		struct Prototype *prototype = CONTAINER_OF(pc, struct Prototype, body[1]);
		emit_const(state, LISP_BYTECODE, TAG_OBJ(prototype_chunk(prototype)));
		...
		break;
	case FHDR_JIT:
		if (!state->parent) { nyi(state); break; } // Await side trace
		guard_value(state, state->bp, *bp);
		state->link = (*ctx->traces)[x.b];
		assemble_trace(state, TRACE_LINK_ROOT);
		return false;
	default:
	...
	if (state->is_aborted) penalize(state);
	return !state->is_aborted;
}
