...
#define IR_TYPE 0x1f
#define IR_TYPE_GUARD 0x20
#define IR_TYPECHECK 0x40 ///< Type guard is non-eliminable.
...
static bool is_nop(union Node x) {
	return x.reg == 0xff && !(x.ty & IR_TYPECHECK || is_effectful(x.op));
}
...
static void print_trace(struct JitState *state, enum TraceLink link) {
	...
	for (unsigned i = REF_FIRST, snap_idx = 0; ; ++i) {
		...
		printf("%.4u %c%c %s %3u ", i - IR_BIAS, x.ty & IR_TYPE_GUARD ? '>' : ' ',
...
#define ARG_REGS (1 << rdi | 1 << rsi | 1 << rdx | 1 << rcx | 1 << r8 | 1 << r9)
	...
	asm_load64(&ctx->as, (arg_regs >>= 4) & 0xf, x.b);
...
do_retry:
	...
	*(ctx.as.p -= 1 + sizeof(int32_t)) = XI_JMP; // Loop backedge placeholder
	...
	case TRACE_LINK_INTERPR:
		...
		asm_load64(&ctx.as, rdx, state->base_offset);
		asm_load64(&ctx.as, rax, snapshot->pc.p);
		state->base_offset = 0;
...
bool jit_record(struct LispCtx *ctx, struct Instruction *pc, LispObject *bp) {
	...
	case JNIL:
		...
		IR_GET(state, ref).ty |= IR_TYPECHECK; // Prevent DCE
	case JMP: break;
	case CALL:
