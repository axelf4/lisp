...
static void take_snapshot(struct JitState *state) {
	...
	uint8_t num_slots = state->base_offset + state->num_slots;
	if (state->num_snapshots >= MAX_SNAPSHOTS / 2
		|| state->num_stack_entries + num_slots >= MAX_SNAPSHOT_ENTRIES / 2)
...
static IrRef sload(struct JitState *state, int slot) {
	...
	state->num_slots = MAX(state->num_slots, slot + 1);
...
static void do_record(void *userdata) {
	...
	case CALL:
		...
		case LISP_CFUNCTION:
			...
			state->num_slots = x.a + 1; // CALL always uses highest register
	...
	if (result) {
		state->num_slots = MAX(state->num_slots, x.a + 1);
		state->bp[x.a] = result;
	}
}
