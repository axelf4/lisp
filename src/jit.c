...
static enum Register reg_def(struct RegAlloc *ctx, Ref ref, RegSet mask) {
	...
	if (x->spill_slot)
		asm_rmrd(&ctx->assembler, 1, XI_MOVrr, reg, rsp, (x->spill_slot - 1) * sizeof x->v);
	return reg;
}
...
static void asm_loop(struct RegAlloc *ctx, uint8_t *asm_end) {
	...
	FOR_ONES(reg, ctx->phi_regs) {
		...
		asm_rmrd(&ctx->assembler, 1, XI_MOVrr, reg, rsp, (in->spill_slot - 1) * sizeof in->v);
...
static void asm_arith(struct RegAlloc *ctx, enum ImmGrp1 op, Ref ref) {
	...
	if (IS_VAR_REF(x.b)) {
		...
		asm_rr(&ctx->assembler, 0, (op << 3) + 1, src, dst);
