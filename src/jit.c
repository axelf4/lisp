...
struct JitState {
	uint16_t len, num_consts;
	uint8_t base_offset, num_slots, frame_depth;
	...
	bool need_snapshot;
	unsigned char num_snapshots, num_stack_entries;
...
static struct LispTrace *assemble_trace(struct JitState *trace) {
	...
	*result = (struct LispTrace) {
		...
		.arity = trace->start->prototype->arity,
...
struct JitState *jit_new(struct Closure *f) {
...
bool jit_record(struct JitState *state, uintptr_t *bp, struct Instruction *pc) {
	...
	case CALL:
		...
		case LISP_CLOSURE: {
			...
			if (proto->arity & PROTO_VARIADIC
				|| fun_value == TAG_OBJ(state->start)) return /* NYI */ false;
	...
	case TAIL_CALL:
		...
		if (fun_value == TAG_OBJ(state->start)) {
			*state->bp = cur; // Avoid reloading the same closure
			dce(state);
			peel_loop(state);
			...
			struct LispCtx *lisp_ctx
				= (struct LispCtx *) ((uintptr_t) fun_value & ~0xffffffffull);
			uint16_t trace_num = lisp_ctx->num_traces++;
			lisp_ctx->traces[trace_num] = trace;
