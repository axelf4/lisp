...
static void subst_snapshot(struct JitState *state, Ref *subst,
	struct Snapshot *snap, struct Snapshot *loopsnap) {
...
#define ARG_REGS (1 << rdi | 1 << rsi | 1 << rdx | 1 << rcx | 1 << r8 | 1 << r9)
	...
	for (unsigned i = x.b; i--;) {
		...
		if (!IS_VAR(arg_ref)) {
			...
			} else asm_mov_mi64(&ctx->assembler, args_reg, i * sizeof arg.v, arg.v);
			continue;
		}
...
static void asm_stack_restore(struct RegAlloc *ctx) {
	...
	FOR_SNAPSHOT_ENTRIES(snapshot, ctx->state->stack_entries, entry) {
		...
		} else asm_mov_mi64(&ctx->assembler, bp, entry->slot * sizeof x.v, x.v);
	}
}
	...
	*(ctx.assembler.p -= 1 + sizeof(int32_t)) = XI_JMP; // Loop backedge placeholder
	...
	for (unsigned i = state->len; i--;) {
		...
		case IR_CAR: {
			...
			asm_rr(&ctx.assembler, 1, IMM_GRP1_MR(XG_ADD), dst, REG_LISP_CTX);
