...
#define BITSET_OP(x, op, a, i) ((x)[(i) / (CHAR_BIT * sizeof *(x))] \
		op (typeof(*(x)))(a) << (i) % (CHAR_BIT * sizeof *(x)))
...
static bool try_pilot(unsigned char pilot, size_t len, uint64_t keys[static len],
	...
	for (size_t i = 0; i < len; ++i) {
		...
		if (BITSET_OP(taken, &, 1, pos)) return false;
	...
	for (size_t b = 0, stack[64], stack_size = 0, recent[16] = {}, recent_idx = 0,
		...
		do {
			...
			for (size_t i = 0; i < bucket->size; ++i) {
				...
				if (!BITSET_OP(taken, &, 1, pos)) continue; // No collision!
		...
		for (size_t i = 0; i < bucket->size; ++i) {
			size_t pos = positions[i]
				= pthash_position(keys[bucket->start + i], pilot, n_prime);
			if (!BITSET_OP(taken, &, 1, pos)) continue;
