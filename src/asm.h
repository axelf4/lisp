...
/** Emits @a op with operands @a reg and `[%base+offset]`.
 *
 * @a op determines which is Operand1 and Operand2 respectively.
 */
static inline void asm_rmro(struct Assembler *ctx, uint8_t op,
	enum Register reg, enum Register base, int32_t offset) {
	enum Mod mod;
    if (!offset && (base & 0b111) != rbp) mod = MOD_DISP0;
    else if ((int8_t) offset == offset) {
		mod = MOD_DISP8;
		*--ctx->p = offset;
    } else {
		mod = MOD_DISP32;
		asm_write32(ctx, offset);
	}
	if (base == rsp) *--ctx->p = SIB(/* scale 1 */ 0, rsp, rsp);
	*--ctx->p = MODRM(mod, reg, base);
	*--ctx->p = op;
	EMIT_REX(ctx, 1, reg, 0, base);
}
...
static inline void asm_call(struct Assembler *ctx, uint8_t *target) {
	asm_write32(ctx, rel32(ctx->p, target));
	*--ctx->p = XI_CALL;
}
#else
...
static inline bool asm_init(struct Assembler *ctx) {
	...
	for (unsigned i = 0; i < JMP_RANGE; ++i) {
		...
		do hint = (state = fxhash64(state, hint)) & ((1u << JMP_RANGE) - 0x10000);
		while (hint + length > 2 * range);
		hint += target - range;
	}
	return false;
}
...
static inline int (*asm_assemble(struct Assembler *ctx))() {
	...
	if (mprotect(beg, MCODE_CAPACITY, PROT_EXEC)) die("mprotect failed");
